...
- **입력 항목**:
  ...
  - **시작 시간**: 24시간 형식 (예: `20:00`)
  - **종료 시간**: 24시간 형식 (예: `22:30`)
    - *참고: 시간은 30분 단위로만 입력 가능합니다. 시작 시간은 종료 시간보다 빨라야 합니다.*
    - *중복 신청 불가: 동일 요일에 이미 신청한 시간대와 겹치는 경우 신청이 거부됩니다.*
...
#### 3. 신청 현황 확인 (`/paring_status`)
현재 회차의 신청자 목록을 확인합니다.
- **사용법**: `/paring_status [round_no]`
  - `round_no`: 확인할 회차 번호 (생략 시 최신 회차)
- **효과**:
  - 신청자들을 요일/시간별로 그룹화하여 보여줍니다.
  - 총 신청자 수를 확인할 수 있습니다.

#### 4. 매칭 실행 (`/paring_match`)
마감된 회차의 신청자들을 매칭합니다.
- **사용법**: `/paring_match [round_no]`
  - `round_no`: 매칭할 회차 번호 (생략 시 최신 회차)
- **효과**:
  - **매칭 우선순위**:
    1. **시작/종료 시간이 정확히 일치하는** 사용자끼리 최우선으로 매칭됩니다.
    2. 그 다음으로 **겹치는 시간이 가장 긴** 사용자끼리 매칭됩니다.
    3. 겹치는 시간이 같다면, **시작 시간이 더 이른** 사용자가 우선 순위를 가집니다.
    4. 한 번 매칭된 사용자는 해당 회차에서 중복 매칭되지 않습니다. (1:1 매칭)
  - 매칭 결과가 저장되고, 당사자들에게 DM이 발송됩니다.
    - **매칭 시간은 겹치는 시간대의 시작부터 30분간 진행됩니다.**
